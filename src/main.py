...
def perform_handshake(meta_info: dict, peer_ip: str, peer_port: str, peer_id: str, peer_socket: socket.socket):
...
def download_piece(piece_index: int, meta_info: dict, peer_socket: socket.socket, piece_outfile: Path):
    block_size = 2**14
    byte_length = 4
    block_reqs = []
    piece_blocks = []
...
def main():
    ...
    if command in ("info", "peers", "handshake", "download_piece") and isinstance(decoded_value, dict):
        ...
        if command == "info":
            ...
            meta_info = get_meta_info(decoded_value)
            ...
            try:
                ...
                meta_info = get_meta_info(decoded_value)
                peer_list = get_peer_list(meta_info, peer_id)
                peer_socket: socket.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                handshake_response: bytes = perform_handshake(meta_info, peer_ip, peer_port, peer_id, peer_socket)
            ...
            try:
                ...
                handshake_response = perform_handshake(meta_info, peer_ip, peer_port, peer_id, peer_socket)
