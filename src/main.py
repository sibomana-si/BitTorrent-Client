...
def download_magnet_piece(magnet_info: dict, peer_socket: socket.socket, piece_index: int) -> bytes:
    try:
        print(f"downloading piece_index: {piece_index} ...")
        block_size = 2**14
        byte_length = 4
...
def download_magnet_file(magnet_info: dict, peer_socket: socket.socket) -> None:
    try:
        torrent_outfile = Path(sys.argv[3])
        print(f"downloading to {torrent_outfile} ...")
